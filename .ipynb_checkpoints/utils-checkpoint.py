...
def generate_two_moons_dataset(dataset_size=10000, noise=0.05):
	...
	for dset in dataset.keys():
		...
		data, labels = make_moons(dataset_size, noise=noise)
		dataset[dset] = TensorDataset(torch.from_numpy(data).float(), torch.from_numpy(labels).long())

	return dataset

def visualize_cluster_assignment(model, dataset, show=False):
...
if __name__ == "__main__":
	...
	generate_two_moons_dataset(10000)